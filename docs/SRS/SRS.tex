...
\begin{figure}[h!]
\begin{center}
 \includegraphics[width=0.8\textwidth]{SystemContext_2.png}
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Label& \bf Multiclass Sum of Cut Edges\\
  \hline
  Input & 
  The input data matrix, $D$, and the class label matrix, $C$, based on \ref{sec:dataConstraints}.  \\\hline
  Output&$J$, $0 \leq J \leq n^2 $, such that\\
  &$J = \sum_{r=1}^{k-1} \sum_{s=r+1}^{k} J_{r,s}$\\
  & $ J_{r,s}  = \sum^{n}_{i=1} \sum^n_{j=1} V_{ij} \times I(c_i = r \And c_j = s)$ \\
  \hline
  Description & The above equation gives the formula for calculating the multiclass sum of cut edges. \\
& $k$ is the number of classes.\\
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Label& \bf Multiclass Sum of Non Cut Edges\\
  \hline
  Input & 
  The input data matrix, $D$, and the class label matrix, $C$, based on \ref{sec:dataConstraints}.  \\\hline
...
\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR:accuracy}:]
  \textbf{Accuracy} As per \aref{A:fp}, any calculation error should be negligible. The accuracy of the computed solutions should meet the level needed for data analysis. The absolute error should be less than 0.01.
...
\item[LC\refstepcounter{lcnum}\thelcnum\label{LC:optWeights}:] The addition of user options such as option to weigh the edge calculations: \iref{IM:SNCE} and \iref{IM:SCE}. Related to: \aref{A:weights}.

\item[LC\refstepcounter{lcnum}\thelcnum\label{LC:optGraphs}:] The addition of user options such as option to change the algorithm for graph construction: \iref{IM:SNCE} and \iref{IM:SCE}. Related to: \aref{A:rng}.

\item[LC\refstepcounter{lcnum}\thelcnum\label{LC:output}:] Changes to the output format. Related to: \aref{A:pvalue}.

\mycomment{Give the likely changes, with a reference to the related assumption (aref), as appropriate.}
...
\ddref{DD:ED}             &X& &  X& & &  \\ \hline
\iref{IM:SCE}             &X& X& X& & & \\ \hline
\iref{IM:SNCE}            &X& X& X& & & \\ \hline
\iref{IM:CEWS}            &X& X& X& & & \\ \hline
...
\hline        
	& \dref{GD:RNG} & \ddref{DD:ED} & \iref{IM:SCE} & \iref{IM:SNCE} & \iref{IM:CEWS}  \\
\hline
\dref{GD:RNG}    & & x& x& x& \\ \hline
\ddref{DD:ED}    & x& & & & \\ \hline
\iref{IM:SCE}    & x& & & &x \\ \hline
\iref{IM:SNCE}   & x& & & &x \\ \hline
\iref{IM:CEWS}   & & &x &x & \\ \hline
\end{tabular}
...
\hline
	& \iref{IM:SCE} & \iref{IM:SNCE} & \iref{IM:CEWS} \\
\hline
\iref{IM:SCE}           &  & &X \\ \hline
\iref{IM:SNCE}          &  & &X \\ \hline
\iref{IM:CEWS}          & x& x& \\ \hline
\rref{R:inputs}         &X &X & \\ \hline
\rref{R:calculate}      &X &X &X \\ \hline
