...
This document uses notation used in \cite{zighed_statistical_2005} and \cite{lorena_how_2020}.
...
The Cut Edge Weight Statistic measurement is a variation of the Fraction of Points on Class Boundary, proposed by Zighed et al. \cite{zighed_statistical_2005}.
The algorithm for the normalized measurement begins by constructing a Toussaint’s Relative Neighbourhood Graph \cite{toussaint_relative_1980} over the entire dataset then computing the number of edges which connect different classes (the "cut edges") over the total number of edges (shown in Figure \ref{fig:RNG}). 
...
\begin{itemize}
\item Observation: An identifiable element in a data set which has features and a class label associated with it.
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A:CWN}:] All observations from one class share the same class label.
...
The user does not want the statistic $p$-value.
...
\begin{itemize}
    ...
    \item In case no error occurs, no error will be raised and $J_N$ will be returned as a float type.
\end{itemize}
...
\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR:accuracy}:]
  ...
  The accuracy of the computed solutions should meet the level needed for data analysis. The absolute error should be less than ACCURACY\_ERR\_ABS. (See Table \ref{Tabel:constants})
