...
\midrule 
$I(...)$ & N/A & Indicator Function, returns 1 if its argument is true and 0 otherwise \\ %Ellipsis
$J_N$ & N/A & Normalized Multiclass Cut Edge Weight Measurement \\
$n$ & N/A & Number of observations \\
$m$ & N/A & Number of features \\
$C$ & N/A & Class label matrix of size $n \times 1$\\
$D$ & N/A & Data matrix of size $n \times m$\\
$V$ & N/A & Graph connection matrix \\
$d(a, b)$ & N/A & Euclidean distance between points $a$ and $b$ \\
\bottomrule
\end{longtable*}
...
This document uses notation the used in \cite{zighed_statistical_2005} and \cite{lorena_how_2020}.
...
Classification is a common machine learning problem, many techniques are used for solving classification problems. 
Unfortunately, not all classification problems are easy and it can be difficult to explain why classifiers perform poorly on certain datasets. 
...
One of these aspects is separability, an intrinsic characteristic that describes how the observations from different classes mix together \cite{guan_novel_2021}. 
...
The Cut Edge Weight Statistic (CEWS) measurement is a variation of the Fraction of Points on Class Boundary, proposed by Zighed et al. \cite{zighed_statistical_2005}.
The algorithm for the normalized measurement begins by constructing a Toussaint’s Relative Neighbourhood Graph \cite{toussaint_relative_1980} over the entire dataset then computing the number of edges that connect different classes (the ``cut edges'') over the total number of edges (shown in Figure \ref{fig:RNG}). 
This measurement is a viable way of measuring separability because it has a high coefficient of determination ($R^2$) with the mean error rate of several classifiers \cite{zighed_statistical_2005}.
CEWS System is a program designed to calculate the CEWS.

\begin{figure}[h!]
\begin{center}
 \includegraphics[width=0.7\textwidth]{RNG_CEWS_Example.png}
\caption{An visualization of the RNG for a binary classification problem represented by black and white points \cite{zighed_statistical_2005}. (a) An example of a 2D RNG graph construction. (b) Identifying cut edges with dotted lines.}
\label{fig:RNG} 
\end{center}
\end{figure}
...
The reader should have a good understanding of classification (at a graduate level or equivalent experience). 
The reader should have a base understanding of data analysis, machine learning and statistics such as from standard undergraduate courses.
The reader should be familiar with basic mathematical number types and their common symbols.
...
\begin{itemize}
\item Provide the input data matrices. (Section \ref{sec:dataConstraints})
\item Correctly format the input data matrices as per Section \ref{sec:dataConstraints}. (For example, it is the responsability of the user to make sure the points and labels are correctly associated as this can not be checked by the system.)
\item Manage the computer's resources including supplying the system with enough resources to support the size of their dataset.
\item Understand the problem.
\item Perform any data pre-processing required for the relevant classification problem such as transformations, feature extraction, etc.
...
\item Detect data type mismatch or if input data violates any constraints. (Section \ref{sec:dataConstraints})
\item Perform the analysis.
\item Perform basic error checking on the output.
...
The end user of CEWS System should have a good understanding of classification and machine learning, preferably with an undergraduate degree in software or mathematics.
...
\begin{itemize}
\item Observation: An identifiable element in a data set that has features and a class label associated with it.
\item Dataset: A set of observations from a population including information about those observations.
\item Feature: An individual measurable property of an observation.
\item Class label: Represents the class an observation belongs to.
\item Classification: The problem of identifying which of a set of categories an observation belongs to.
\item Data complexity: Aspects of the data that classifiers have difficultly with \cite{luengo_automatic_2015}.
\item Separability: A type of data complexity that measures how the observations from different classes mix together \cite{guan_novel_2021}.
\item Boolean variable: A variable that has one of two possible values.
\item Data Matrix: a matrix containing the features for each observation.
\item Class Matrix: a matrix containing class labels.
\end{itemize}
...
\noindent Given the dataset, the goal statement is:

\begin{itemize}
\item[GS\refstepcounter{goalnum}\thegoalnum \label{G:CEWS}:]  Calculate the normalized multiclass CEWS measurement.
\end{itemize}
...
Section~\ref{sec:instance}.  The information to understand the meaning of the
...
All rational numbers can be represented in a base-2 computer system. Operations performed on rational numbers in a computer system is exact. Any computation rounding error is negligible and does not need to be taken into account.

\item[A\refstepcounter{assumpnum}\theassumpnum \label{A:CWN}:] All observations from one class share the same class label. (For example, no subclasses exist.)

%\item[A\refstepcounter{assumpnum}\theassumpnum \label{A:NOF}:]
%Each observation has the same number of features. No features are null. 

\item[A\refstepcounter{assumpnum}\theassumpnum \label{A:weights}:]
Weights are not used for the edge calculation.

\item[A\refstepcounter{assumpnum}\theassumpnum \label{A:rng}:]
A relative neighbourhood graph is used as the base graph.

\item[A\refstepcounter{assumpnum}\theassumpnum \label{A:pvalue}:]
The statistic $p$-value is not needed.
...
% #5 description
{
Based on assumptions \ref{A:fp}, \ref{A:CWN}, \ref{A:NOF}, \ref{A:weights}, \ref{A:rng} and \ref{A:pvalue}.
...
This section collects the equations that will be used in building the instance models.
...
SI Units& N/A \\ \hline
Equation & Connection matrix: $V = (v_{ij}), i = 1,2, ... ,n; j = 1,2, ... , n;$ \\
& where $v_{ij} = I([ d(p_i,p_j) \leq \max (d(p_i,c), d(p_j,c)) \forall c , c \in P, c \neq p_i, c \neq p_j ])$\\
 \hline
Description &
...
& $P$ is a set of $n$ points in real space with dimension(s) $m$ with elements $p_{i}$: $i=1,...,n$.\\
& $V$ is the connection matrix that represents the RNG where $v_{ij} =  1$ if there is an edge and $v_{ij} =  0$ if not.\\
& $d(a,b)$ is the Euclidean distance between points $a$ and $b$ given by \ddref{DD:ED}.\\
& Based on assumptions \ref{A:fp}. \\
\hline
...
Symbol &$d(a,b)$\\ \hline
  ...
  Description & 
                ...
                $a_i$ and $b_i$ are vectors from the origin. 
                Based on assumption \ref{A:fp}. 
  \\ \hline
  Sources& N/A \\ \hline
...
The goal GS\ref{G:CEWS} is solved by \iref{IM:CEWS}, which is the implementation of the cut edge weight statistic measurement.
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
    ...
    \hline
    Input & The input data matrix, $D_{n \times m}$, and the class label matrix, $C_{n \times 1}$, based on \ref{sec:dataConstraints}. \\ \hline
    Output&$J_{N}$, $0\leq J_N \leq 1$, such that\\
    &$J_{N} = \frac {J}{I+J}$\\
    & where \\
    & $J = \sum_{r=1}^{k-1} \sum_{s=r+1}^{k} J_{r,s}; 0 \leq J \leq n^2$\\
    & where $J_{r,s}  = \sum^{n}_{i=1} \sum^n_{j=1} V_{ij} \times I(c_i = r \And c_j = s)$ \\
    & $I = \sum_{r=1}^{k} I_r, 0\leq I \leq  n^2$ \\
    & where $I_{r}  = \sum^{n-1}_{i=1} \sum^n_{j=i+1} V_{ij} \times I(c_i = r \And c_j = r)$ \\
    \hline
    Description &
    Based on assumptions \ref{A:fp}, \ref{A:CWN}, \ref{A:weights}, \ref{A:rng} and \ref{A:pvalue}. \\
    & The above equation gives the formula for calculating the the multiclass normalized cut edge weight measurement. \\
    & $k$ is the number of classes.\\
    & $V$ is the RNG connection matrix for matrix $D$ as $P$ given by \dref{GD:RNG}.\\
    & $v_{ij}$ is element $i, j$ of $V$.\\
    & $c_i$ is element $i$ of $C$.\\
    \hline
...
Table~\ref{TblInputVar} shows the data constraints on the input variables.  
...
The user provides a dataset in the format of matrices $D$ and $C$. 
...
\] \\
\begin{minipage}{\textwidth}
So row $i$, $d_i$, contains observation $i$.
...
\] \\
\end{minipage}
Where $c_i$ is the label for observation $i$ of matrix $D$. 
It is the user's responsibility to set class label $c_i$ to the class of observation $d_i$. (Section \ref{sec:systemContext})
...
\noindent
A correct solution must exhibit the following behaviour: Table \ref{tabel:OutputVar}.
...
\begin{table}[H]
\caption{Output Variables} \label{tabel:OutputVar}
\renewcommand{\arraystretch}{1.2}
\noindent \begin{longtable*}{l l} 
  \toprule
...
\begin{itemize}
    \item Any constraint in \ref{sec:dataConstraints} is violated, the CEWS System will generate an appropriate error message. 
\end{itemize}
...
% Do calculation
\item[R\refstepcounter{reqnum}\thereqnum \label{R:calculate}:] After reading the input data, CEWS System will perform the calculation in \iref{IM:CEWS} and output the result.
\begin{itemize}
    %\item The CEWS System should generate an error if the calculation result exceeds the constraints in \ref{sec:correctSolution}.
    \item In case an error occurs, an error will be raised by the program.
    \item In case no error occurs, $J_N$ will be the output.
\end{itemize}
...
% Reproducible
\item[R\refstepcounter{reqnum}\thereqnum \label{R:repro}:] The CEWS System will generate the exact same output for the exact same dataset.
\begin{itemize}
    \item Including if the order of the dataset changes, as long as the same labels are associated with the same points.
\end{itemize}
...
\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR:accuracy}:]
  \textbf{Accuracy} 
  %As per \aref{A:fp}, any calculation error should be negligible. 
  The accuracy of the computed solutions should meet the level needed for data analysis. 
  The absolute error should be less than ACCURACY\_ERR\_ABS. (See Table \ref{Tabel:constants})
...
%\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR:usability}:] \textbf{Usability} A program should be able to call CEWS System by calling a function of the form CEWS($D$, $C$) and CEWS System should return the output.
...
\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR:port}:]
  \textbf{Portability}
    The software should run any machine running the Windows 11 operating system.
...
\hline
	& \aref{A:fp} & \aref{A:CWN} & \aref{A:weights} & \aref{A:rng} & \aref{A:pvalue} \\
\hline
\dref{GD:RNG}             &X& &X& &  \\ \hline
\ddref{DD:ED}             &X& & & &  \\ \hline
\iref{IM:CEWS}            &X&X&X&X&X \\ \hline
...
\rref{R:repro}          &X \\ \hline
\nfrref{NFR:accuracy}   &X \\ \hline
\nfrref{NFR:port}  &X \\ \hline
\end{tabular}
