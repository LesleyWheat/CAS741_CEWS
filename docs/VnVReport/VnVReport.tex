...
\begin{itemize}
    ...
    \item Result: Pass*
    \item *Issues with existing implementations may invalidate some failed test cases (see Section \ref{sec:comparision}).
    \end{itemize}
...
\begin{itemize}
    ...
    \item NFR2: Portability 
    \begin{itemize}
    \item Description: The software should run any machine running the Windows 11 operating system.
    \item Result: Pass
    \end{itemize}
\end{itemize}
...
Note: the CEWS calculated with the graph from \cite{pythonRNG} does not match the results from \cite{zighed_statistical_2005}.
...
However, the \cite{pythonRNG} has not been independently verified.
...
Another common data complexity measure, Fraction of Borderline Points uses a Minimum Spanning Tree and nearest neighbour graphs are often used as well \cite{lorena_how_2020}.
As such the program may still be able to measure data complexity even if the program doesn't reproduce the exact same result which is why there is a margin of error allowed.
...
Some of the datasets were generating very different results from the \cite{zighed_statistical_2005}, this is likely due to differences in the dataset processing.
When the process to normalize the datasets was changed, the program passed T11.
It is unclear what type of normalization was used in \cite{zighed_statistical_2005} and the normailzation now used may not be exactly the same but the results are now within the accuracy range.
